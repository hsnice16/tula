...
export async function about(connectors: Map<string, Connector>): Promise<CommandResult> {
  ...
  return {
    output: [
      ...
      '',
      'It cannot place an order, move funds, or hold a private key. No code path',
      'exists that can, and the build fails if one appears. It never asks for a',
      'seed phrase. The model narrates these numbers; it never computes them and',

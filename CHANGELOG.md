...
- **Nothing moves under the cursor.** The command menu moved below the input line and holds a fixed height, so narrowing it from twenty-one commands to three no longer collapses the block and drags the line you are typing on up the screen. ctrl+k and ctrl+o open as panels one row short of the viewport, so each is the only thing on screen while it is open, and closing one leaves the scrollback it was covering intact.
- **Long output is kept to twelve lines, with ctrl+o for the rest.** A 176-position book was a screenful per answer, and the question that produced it scrolled off before it could be read beside what it returned. What is held back is counted, never dropped; ctrl+o opens that entry in a scrollable pane, and pressing it again walks back through the earlier ones.

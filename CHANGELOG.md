...
- **[A published site](https://hsnice16.github.io/tula/)** — overview, install, roadmap, changelog and the security model. The changelog and roadmap pages are read, not written: they render this file and every `**Status**:` line under `tasks/` at build time, so a published status cannot drift from the repository. A separate package with its own lockfile, so its dependency tree never joins the binary's.
